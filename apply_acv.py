...
def apply(lut, courbe):
    ...
    if len(courbe) > 4:
        order, value = [(2, 0)], [(2, 0)]  # natural spline boundary conditions
        fct = make_interp_spline(courbe[::2],
                                 courbe[1::2],
                                 k=3,
                                 bc_type=(order, value))
...
def apply_all(image, courbes, masque):
    ...
    if masque is not None:
        a_1 = masque/255.
        a_2 = (-masque+255)/255.
        image[0] = np.round(np.multiply(a_1, np.take(lut_r, image[0]))
                            + np.multiply(a_2, image[0]))
        image[1] = np.round(np.multiply(a_1, np.take(lut_v, image[1]))
                            + np.multiply(a_2, image[1]))
        image[2] = np.round(np.multiply(a_1, np.take(lut_b, image[2]))
...
for line in open(fichier):
    ...
    for i in range(nbAcv):
        ...
        if len(T[2+2*num_acv]) > 2:
            nom_alpha = os.path.join(dir_acv,
                                     T[2+2*num_acv]).replace('.psb', '.tif')
